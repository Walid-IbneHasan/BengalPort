<script lang="ts">
  ...
  async function submitGeneral() {
    sending = true; error = "";
    try { await api("/enquiries", { method: "POST", body: JSON.stringify({ type: tab === "UMRAH" ? "UMRAH" : "GENERAL", name, phone, email, message, details: { subject, service: tab === "UMRAH" ? "UMRAH" : "GENERAL" } }) }); success = "Thank you. Your enquiry has been received and our team will contact you."; name = phone = email = subject = message = ""; }
    catch (e) { error = e instanceof Error ? e.message : "Could not submit the enquiry."; }
    finally { sending = false; }
  }
</script>

<svelte:head><title>Apply / Enquiry | Bengal Port</title><meta name="description" content="Submit a Bengal Port business, education or healthcare application, or send a general or Umrah enquiry."/></svelte:head>
...
<section class="application-section"><div class="wrap">
  ...
  {#if tab === "GENERAL" || tab === "UMRAH"}
    <div class="general-box"><div class="general-intro"><span>{tab === "UMRAH" ? "UMRAH ENQUIRY" : "QUICK ENQUIRY"}</span><h2>{tab === "UMRAH" ? "Plan your sacred journey." : "Ask a question without registering."}</h2><p>{tab === "UMRAH" ? "Share your preferred travel period, traveller count and priorities. No account is required." : "Only the essentials are required. Login is optional."}</p></div>
